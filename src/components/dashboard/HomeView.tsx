...
function Counter({ value, duration = 2, label }: CounterProps) {
  ...
  return (
    ...
    >
      <motion.div 
        className="text-2xl md:text-3xl font-bold text-gray-900 group-hover:text-[#0284a5] transition-colors"
      >
        {displayValue}
      </motion.div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-112px)] pt-6 pb-16 px-4 md:px-6 lg:px-8">
      ...
      <div className="lg:col-span-3 h-[calc(100vh-112px)] overflow-y-auto pb-16 scrollbar-hide">
        ...
        <div className="bg-white rounded-xl border border-[#e1e3e5] p-6 mb-8 animate-fadeIn">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {loader ? (
              ...
              <>
                <Counter value={42} label="AI Models" />
                <Counter value={78} label="Datasets" />
                <Counter value={120} label="Total Assets" />
                <Counter value={356} label="Transactions" />
              </>
            )}
          </div>
        </div>

        {/* Trending Section - Side by Side Layout */}
                ...
                <>
                  {modelData?.length > 0 ? modelData?.slice(0, 5).map((option: any, index: number) => (
                    <div
                      ...
                      className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg transition-colors cursor-pointer border border-[#e1e3e5]"
                    >
                  ...
                  {modelData?.length > 5 && (
                    <div className='text-gray-900 flex justify-center'>
                      <button 
                        ...
                        className='bg-primary font-medium p-2 rounded-lg cursor-pointer text-white w-fit hover:bg-[#026d8a] transition-colors'
                      >
                        See more models
                      </button>
                    </div>

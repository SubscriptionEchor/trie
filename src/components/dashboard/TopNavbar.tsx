...
export function TopNavbar() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let getTokenCount = await END_POINTS.get_ft_info_by_did({ did: connectedWallet?.did }) as any
      if (getTokenCount?.status && getTokenCount?.ft_info?.length > 0) {
        setFtCount(getTokenCount?.ft_info[0]?.ft_count)
      }
  ...
  useEffect(() => {
    if (isAuthenticated && messageHandlerRef.current) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (searchRef.current && !searchRef.current.contains(event.target as Node)) {
        setSearchQuery(""); // Close dropdown if clicked outside
      }
    };
  ...
  const onSearch = (value: string) => {
      ...
      setSearchData(
        nftData
          ?.filter((data: any) =>
            ...
            data.usageHistory.length > 0 && data?.metadata?.name?.toLowerCase().includes(value?.toLowerCase())
          )
      )
    }
    if (selectedOption?.id == "infra") {
      setSearchData(
        infraProviders
          ?.filter((data: any) => data?.name?.toLowerCase()?.includes?.(value?.toLowerCase())
          )
      )
    }
  ...
  const onClickResult = (data: any) => {
    let slug = data?.metadata?.name || data?.name as any
      ...
      <div className="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 relative">
          ...
          <div className="flex items-center space-x-6">
            ...
            {connectedWallet?.did ? <div className="text-white text-sm font-medium">TRIE Balance: {ftCount} </div> : null}
